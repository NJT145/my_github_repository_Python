...
class crawler:
    ...
    def addtoindex(self, url, soup):
        ...
        # return True    
        # ---------------------------------
        return True, self.getWordFrequency(words) # return a dict that consist of frequency for each word
        # ---------------------------------
    ...
    def crawl(self, pages, depth=2):
        ...
        for i in range(depth):
            ...
            for page in pages:
                ...
                # -------------------------------------------
                added, words_frequency = self.addtoindex(page, soup)  # get frequency of words per url
                # -------------------------------------------
                ...
                # -------------------------------------------
                wordsFrequency_perUrl[page] = words_frequency  # and add it to the dict of wordsFrequency_perUrl.
                # -------------------------------------------    
                ...
                for link in links:
                    if 'href' in dict(link.attrs):
                        ...
                        if url[0:4] == 'http' and not self.isindexed(url) and 'Bolum' in url:
                            newpages.add(url)
        ...
        # --------------------------
        return wordsFrequency_perUrl
        # --------------------------

    def getWordFrequency(self, text):
            word_frequency = dict()
            for word in text:
                if word in ignorewords:  # ignore, if that word exists in the list of ignorewords.
                    continue
                if word not in word_frequency: # if not in dict
                    word_frequency[word] = 1 # then add it with the value of 1
                else: # if it is already inside
                    word_frequency[word] += 1 # then increment its value by 1
            return word_frequency

    def createWordFrequencyMatrix_perUrl(self, urls_wordsFrequency, file2):
        words = list()
        for words_frequency in urls_wordsFrequency.values(): #  we get the value for each url
            for word in words_frequency.keys(): #  get the words
                    if word not in words: # if that word not in the list of words
                        words.append(word) # add it then.
        a = list()
        a.extend(["Urls"] + [word for word in words]) #  this is for the first row of the file
        out = csv.writer(open(file2,"a"), delimiter='\t', quoting=csv.QUOTE_NONE) #  we use csv. module instead of file()
        out.writerow(a) #  write this line to the file
        del a[:] #  and clean the list for the next usages
        for url, words_frequency in urls_wordsFrequency.items(): #  then for the url:words_frequency pairs
            w = []
            for word in words:
                if word not in words_frequency: w.append("0") # if that word is not in the value of that particular url, then assign "0" the its value
                else: w.append(words_frequency[word]) #  its original value otherwise
            a.extend([url] + w)
            out.writerow(a)
            del a[:]
