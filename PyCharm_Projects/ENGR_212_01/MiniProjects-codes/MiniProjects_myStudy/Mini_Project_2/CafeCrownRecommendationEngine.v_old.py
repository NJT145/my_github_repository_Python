...
* for getting an appropriate self.value_of_combo, i took a look to :
...
class CafeCrownRecommendationEngine(Frame):
    ...
    def initUI(self):
        ...
        self.box.pack()
        frame11.pack(side=LEFT, fill=BOTH, expand=True, padx=20, pady=20)
        #"Enter your rating" inner-frame
        frame12=Frame(frame1)
        Label(frame12, text="\nEnter your rating:   \n", fg='red', font='Arial 12 bold').pack()
        self.scale=IntVar()
        Scale(frame12, from_=0, to=10, length=200, orient=HORIZONTAL, variable=self.scale).pack()
        frame12.pack(side=LEFT, fill=BOTH, expand=True, padx=20)
        # user ratings add/remove
        frame13=Frame(frame1)
        Button(frame13, text="Add", font='ArialBlack 10 bold', fg='blue', command=self.onClickAdd).pack(side=LEFT, padx=20)
        listBoxWithScrollbar=Frame(frame13)
        scrollbar=Scrollbar(listBoxWithScrollbar)
        self.mylist=Listbox(frame13, yscrollcommand=scrollbar.set)
        ...
        Button(frame22, text="Get Recommendations", font='ArialBlack 10 bold', fg='blue', command=self.onClickGetRecom).pack(side=LEFT)
    ...
    def onClickAdd(self):
        if self.value_of_combo == None:
            ...
            print self.mylist.get(0)

    def onClickRemoveSelected(self):
        self.ownratings = open("ownratings.db", 'c')

        print self.mylist.size()
        print str(self.mylist.get(END))
